...
const BRIDGE_CAPABILITIES = [
  ...
  'platform.loginCheck', 'platform.selectAccount',
...
function bridgeEvent(commandId, event) {
  const key = String(commandId || '');
  const sequence = (bridgeEventSequences.get(key) || 0) + 1;
  bridgeEventSequences.set(key, sequence);
  return bridgeQueue.addEvent({ ...event, commandId: key, sequence });
}

function bridgeCommandView(commandId) {
  const command = bridgeQueue.get(commandId);
  if (!command) return null;
  const snapshot = bridgeQueue.snapshot();
  return {
    command,
    events: snapshot.events.filter(event => event.commandId === String(commandId))
  };
}
...
app.get('/api/bridge/capabilities', (req, res) => {
  res.json({ ok: true, protocolVersion: 1, capabilities: BRIDGE_CAPABILITIES });
});

app.post('/api/bridge/commands', (req, res) => {
  try {
    const command = bridgeQueue.enqueue(req.body || {});
    bridgeEvent(command.id, { type: 'accepted', action: command.action, status: 'queued' });
...
app.get('/api/bridge/commands/:id', (req, res) => {
  const view = bridgeCommandView(req.params.id);
  if (!view) return res.status(404).json({ ok: false, message: '命令不存在' });
  res.json({ ok: true, ...view });
});

app.post('/api/bridge/commands/:id/events', (req, res) => {
  if (!bridgeQueue.get(req.params.id)) return res.status(404).json({ ok: false, message: '命令不存在' });
  const event = bridgeEvent(req.params.id, req.body || {});
  res.json({ ok: true, event });
});

app.post('/api/bridge/commands/:id/result', (req, res) => {
  const result = bridgeQueue.resolve(req.params.id, req.body || {});
  if (!result) return res.status(404).json({ ok: false, message: '命令不存在' });
  bridgeEvent(req.params.id, { type: 'result', status: result.status, success: result.result?.success !== false });
  res.json({ ok: true, command: result });
});

app.post('/api/bridge/commands/:id/cancel', (req, res) => {
  const result = bridgeQueue.cancel(req.params.id);
  if (!result) return res.status(404).json({ ok: false, message: '命令不存在' });
  bridgeEvent(req.params.id, { type: 'cancelled', status: 'cancelled', reason: req.body?.reason || '已取消' });
  res.json({ ok: true, command: result });
});
