...
class WeixinPublisher {
  ...
  async publishSingleVideo(video, settings, videoPath, videoIndex, totalVideos) {
    ...
    // ── 步骤3: 上传视频 ──
    step('获取视频文件...');
    const file = await this.getVideoFile(videoPath, video.name, video.path);
    ...
    // ── 步骤10: 声明原创 ──
    if (videoInfo.isOriginal || settings.isOriginal) {
      step('声明原创...');
  ...
  async simulateDragUpload(videoPath, videoName, directPath = '') {
    const file = await this.getVideoFile(videoPath, videoName, directPath);
    if (!file) {
      return null;
    }
  ...
  async getVideoFile(videoPath, videoName, directPath = '') {
    const fullPath = directPath || (videoPath.endsWith('/')
      ? `${videoPath}${videoName}`
      : `${videoPath}/${videoName}`);
