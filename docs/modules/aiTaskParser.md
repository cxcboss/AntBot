...
| topics | `['#动画','#奇葩游戏','#游戏','#小游戏','#休闲游戏']`（与插件硬编码一致） |
...
- 旧逗号格式（`任务名，是否原创，链接`）走规则路径时由本模块内部逻辑复现旧行为（`parseTaskLine` 位于旧模块 `parser.js`，本模块仅 import 其 `parseTaskInput`/`parsePublishTime`/`PLATFORM_KEYS`），结果与旧版一致
