...
| 版本 | 日期 | 修改内容 |
|------|------|---------|
| 0.9.2 | 2026-08-22 | Win 桥接连接稳定性：插件/App 双端并行端口发现（串行 20s→<1s）、成功地址持久化、extensionConnected 窗口 10s→30s、MV3 alarms 顶层同步注册、轮询自适应退避；随包发布浏览器插件 v1.2.0 |
| 0.9.1 | 2026-08-20 | 修复监控输入校验：YouTube 主页支持 /featured /shorts /streams 等后缀并归一化到频道主页，解决 `@xxx/featured` 添加失败 |
| 0.9.0 | 2026-08-20 | 主控去重与重新发布收敛：重试共享 logicalTaskId、历史/持久化/实时跨来源去重、历史重发布回写、重新发布并发锁；远程主控筛选/搜索/停止全部、桌面端同步 |
| 0.8.9 | 2026-08-20 | 监控功能升级：支持 YouTube/TikTok 公开账号；每个监控可选择仅下载、下载并剪辑、下载剪辑并发布；桌面端与远程网页端共用监控配置、SSE 同步；任务卡片展示来源/动作并保留稳定成品与重试快照 |
